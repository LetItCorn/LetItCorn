...
<template>
  <div class="d-flex flex-column gap-3">
    ...
    <table class="table table-borderless mb-0">
      <tbody>
        <tr><th>코드</th><td><input v-model="eq.equipment_code" class="form-control " readonly /></td></tr>
        <tr><th>장비명</th><td><input v-model="eq.equipment_name" class="form-control " /></td></tr>
        <tr><th>유형코드</th><td>
          <select v-model="eq.equipment_type" class="form-select ">
            <option value="">-- 선택하세요 --</option>
            <option v-for="c in typeList" :key="c.code_values" :value="c.code_values">{{ c.code_name }}</option>
          </select>
        </td></tr>
        <tr><th>설치일</th><td><input v-model="eq.install_date" type="date" class="form-control " /></td></tr>
        <tr><th>제조사</th><td><input v-model="eq.manufacturer" class="form-control " /></td></tr>
        <tr><th>다음점검일</th><td><input v-model="eq.next_inspection_dt" type="date" class="form-control " /></td></tr>
        <tr><th>적합여부</th><td>
          <select v-model="eq.is_suitable" class="form-select ">
            <option value="">-- 선택하세요 --</option>
            <option v-for="c in suitableList" :key="c.code_values" :value="c.code_values">{{ c.code_name }}</option>
          </select>
        </td></tr>
        <tr><th>단위코드</th><td>
          <select v-model="eq.unit_code" class="form-select f">
            <option value="">-- 선택하세요 --</option>
            <option v-for="c in unitList" :key="c.code_values" :value="c.code_values">{{ c.code_name }}</option>
          </select>
        </td></tr>
        <tr><th>용량/수량</th><td><input v-model="eq.qty" type="number" class="form-control " /></td></tr>
      </tbody>
    </table>
